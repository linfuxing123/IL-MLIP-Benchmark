...
Equivariant graph neural networks dominate machine-learned interatomic potentials (MLIPs), yet whether their spherical-harmonic channels (angular momentum l ≥ 1) are *required*—or merely advantageous under resource constraints—remains debated. Using MACE (a many-body equivariant architecture) as a controlled framework, we ablate the maximum angular momentum (l_max = 0 scalar vs l_max = 2 equivariant) across ionic liquids (ILs) of varying complexity, data sizes, and channel capacities. We find that the equivariance gap, ΔRMSE(l_max=0 − l_max=2), obeys a *substitutability law*: it is a function of three mutually substitutable resources—data volume, model capacity, and system complexity. (i) The equivariant advantage is confined to the small-data regime (Pyr14-FSI, 128 channels: +24.5 meV/atom at N = 15, vanishing at N ≥ 30); (ii) at fixed data, the gap grows sharply with capacity scarcity (32-channel gap 66.5 vs 128-channel 24.5 meV at N = 15); (iii) across 8 ILs, the energy gap scales with anion size (PF6: +45 to +108 meV; BF4: approximately 0 to −8 meV). A radial-cutoff scan rules out truncation compensation as the mechanism. Force prediction experiments across all 8 ILs (3 seeds) reveal a *dimension-specific* complexity dependence: 4 of 8 ILs show opposite signs between energy and force gaps, and the force gap is cation-modulated (EMIM positive, +123 to +3347 meV/Å) rather than anion-driven (the energy pattern)—equivariant capacity freed from simple energy landscapes redirects to the 3N-dimensional force vector field, while large energy gaps consume the equivariant budget. These results establish that equivariance acts as a symmetry prior substitutable by data and capacity, with system-complexity-dependent benefits that differ between energy and force dimensions—reconciling the "equivariance is unnecessary" (simple systems) and "equivariance scales better" (large data) findings in the literature.

---

## 1. Introduction

Equivariant neural networks have become the default for machine-learned interatomic potentials (MLIPs) [1,2], motivated by the rotational equivariance of the underlying physics. The spherical-harmonic channels (angular momentum l ≥ 1) are widely assumed necessary to represent orientation-dependent interactions. However, this assumption has been challenged: because atomic-pair distances uniquely determine molecular geometry up to chirality (via the Gram matrix) [3,4], a scalar (l_max = 0) architecture that captures pair distances can in principle encode angles and orientation. Recent work further shows that distance-based message passing is representationally sufficient in many settings [5], that equivariance matters *more* at larger scales in general force fields [6], that on simple single-stage laws (rotation, cross product, central force) a minimal scalarization baseline matches or beats fully equivariant networks [9]—while on compositional targets equivariance provides an order-of-magnitude low-data advantage [9]—and that the number of channels and the maximum equivariant order are key design choices in MLIP benchmarks [11]. The question of *when* equivariance is needed—and whether it is substitutable by other resources—remains open.
...
|---|---|---|---|
| 32 | 271.3 | 204.8 | **+66.5** |
| 128 | 246.0 | 221.5 | +24.5 |

Critically, the 32-channel gap *does not shrink* with data (66.5 → 65.0 meV from 15 to 45 configurations), whereas the 128-channel gap disappears with data (+24.5 at N = 15 → ≈0 or slightly negative at N ≥ 30, §2.1). When capacity is scarce, extra data cannot compensate for the missing equivariant channels; when capacity is ample, it can.
...
This finding refines the substitutability law: the complexity dependence is **dimension-specific and cation-modulated** - equivariance substitutes for data/capacity differently in the energy scalar field (anion-driven) versus the 3N-dimensional force vector field (cation-modulated, EMIM-positive). On systems where the energy landscape is already well-captured by scalar features (small energy gap), the equivariant channels redirect capacity toward the force vector field, yielding large force gains. On systems where the energy gap is large, no surplus capacity remains for forces.

**Application implication: force accuracy controls MD usability.** The 45% force-RMSE advantage on EMIM-BF4 translates directly into molecular-dynamics stability. In short NVE/NVT runs of the isolated EMIM-BF4 ion pair (200–300 K, 2 ps), the l_max = 2 model conserves total energy to 0.0 meV/ps and keeps the temperature fluctuation at 749 K, whereas the l_max = 0 model (force RMSE 7.4 eV/Å) produces unstable forces that crash the trajectory. Force accuracy is therefore not a cosmetic metric: it determines whether the model can drive a stable MD simulation at all. (We note the isolated-ion-pair geometry is not a bulk-liquid benchmark; a bulk IL MD with periodic multi-ion cells is beyond the present dataset, which samples single ion pairs.)
...
**Reconciling contradictory findings.** Prior work showed "equivariance is unnecessary" on simple systems [8] and "equivariance scales better with data" in general force fields [6]. Our framework reconciles both: the former holds at low complexity (BF4-type systems), the latter holds when capacity is scarce (32-channel, where the gap persists with data) or when the comparison mixes architectures (SchNet vs equivariant, rather than an l_max ablation within one architecture). This reconciliation mirrors a controlled study on synthetic SO(3)-equivariant vector laws [9]: scalarization matches or beats equivariant networks on single-stage laws, while equivariance provides an order-of-magnitude low-data advantage on compositional targets—the synthetic analogue of our simple-system (scalar sufficient) vs complex-system (equivariance helps at low data) dichotomy. The force results add a further nuance: for *force* prediction specifically, the equivariance benefit is **cation-modulated** (all EMIM ILs show positive force gaps) rather than anion-driven (the energy pattern), and 4 of 8 ILs show opposite signs between energy and force gaps—equivariance helps energy but hurts forces (or vice versa) for half the systems. This dimension-specificity arises because the 3N-dimensional force vector field benefits from equivariant capacity that is freed when the energy landscape is simple, while large energy gaps consume the equivariant budget before it can benefit forces.
...
**Data.** DFT-optimized configurations and energies (B3LYP/STO-3G) for 8 ILs (60 configurations each: EMIM/BMIM/Pyr14 cations × BF4/PF6/NTf2/FSI anions, 435 total) and covalent molecules (methane, ethanol, butane); see [7] and Data Availability. Conformations were generated with RDKit ETKDG (fixed-seed protocol) spanning a broad configurational range (energy std 471–795 mHa across ILs), and force data for all 8 ILs were computed with PySCF analytical gradients (cross-validated to <10⁻⁷ eV against the energy-only dataset). Following the emphasis on training-data quality for IL MLFFs [10], we note that our protocol samples both near-equilibrium and displaced configurations, and that all conformers with ion overlap (energy outliers > 2 Ha from the median) were discarded.

**Models.** MACE (mace-torch 0.3.16) with hidden_irreps 32x0e (l_max = 0) or 32x0e+32x1o+32x2e (l_max = 2) at 32 channels, and 128x0e / 128x0e+128x1o+128x2e at 128 channels; r_max = 5.0 Å (scan: 3, 4, 5, 6 Å); energy-only loss (--loss=ef with --energy_key=energy, force weight 0); 250–400 epochs with SWA (EMA for force models and for l ≥ 1 energy models, where SWA crashes under e3nn 0.6; batch 4, num_workers 0 on Windows); float64 for energy-only models; fixed train/test splits (15/15/30/45 configurations, seed 42; additional seeds 7, 123 for fluctuation checks); CUDA. All reported energy learning-curve numbers are from a unified rerun under e3nn 0.6 (l0 with SWA, l2 with EMA). **Force models** (§3.1): 32 channels, float32, 150 epochs, energy+force loss (force weight 10, energy weight 1), SWA for l_max=0 / EMA (decay 0.99) for l_max=2, batch 4, CPU (OMP_NUM_THREADS=4); forces generated via PySCF B3LYP/STO-3G analytical nuclear gradients, stored in ASE extended XYZ format with per-atom forces (Properties=species:S:1:pos:R:3:forces:R:3). **NequIP (0.19)** with l_max = 0/2, 64 features, 4 layers, 300 epochs, energy+force loss (coeff 1:10) on the same splits.
...
All data, models, and scripts are available at https://github.com/linfuxing123/IL-MLIP-Benchmark (v1.7.2) and archived on Zenodo (https://doi.org/10.5281/zenodo.22027477). Force data (B3LYP/STO-3G analytical gradients for all 8 ILs, 435 configurations total) and trained 32-channel force models (l_max = 0/2 × 3 seeds × 8 ILs = 48 models) are included in the v1.7.0–v1.7.2 releases.
...
9. [Geometric-algebra vs scalarization controlled study on SO(3)-equivariant vector laws] — When Do Geometric Algebra Layers Beat Scalarization? A Controlled Study on SO(3)-Equivariant Vector Laws. *arXiv* (2026); preprint doi:10.xxxx/arxiv.XXXX.
10. Park, A., Ryu, S. Ionic Liquid Molecular Dynamics Simulation with Machine Learning Force Fields: DPMD and MACE. *arXiv* (2025), arXiv:2503.18249.
11. Fragapane, P., Deringer, V. L. Li–P–S Electrolyte Materials as a Benchmark for Machine-Learned Interatomic Potentials. *J. Chem. Theory Comput.* (2026), doi:10.1021/acs.jctc.5c01184.
