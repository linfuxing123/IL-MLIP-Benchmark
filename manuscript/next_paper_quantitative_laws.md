...
*Verification status (honest)*: the three limits reduce correctly; the cross-system trend (EMIM ≈ 0) is confirmed; C\* is located in the (32, 128) interval from the existing 32/128-ch data. The sharp-vs-smooth distinction requires intermediate capacities (64/96 ch), but the legacy 32/128-ch energy learning curves (trained in an earlier mace/e3nn environment) are not reproducible in the current environment (0.4.4/0.5/0.6 all fail: legacy models load with 40× drifted predictions or fail to load; fresh energy-only training underfits — 64-ch l0 8950–10472 meV across losses/epochs, with 1000 epochs unchanged). The candidate law is therefore strong on the verified axes but its smoothness refinement is blocked by environment irreproducibility (future work: either environment archaeology of the 08-18 mace/torch/e3nn stack, or a force-based cross-capacity verification, since force training is reliable under e3nn 0.6).
