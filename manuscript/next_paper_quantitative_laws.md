...
**Cross-system test (κ dependence).** To test whether the critical capacity is set by chemical complexity κ (as the law predicts C\* = λκ/s\^m), we repeat the 32/64/128-ch capacity sweep on a second IL, Pyr14-NTf2 (same Pyr14 cation, different NTf2 anion vs the FSI of the reference). At **32 ch, Pyr14-NTf2 already shows data substitution: gap 52.7 → 16.2 meV/atom (β_eff ≈ 1.07)**, in sharp contrast to Pyr14-FSI at 32 ch (gap 66.5 → 65.0, β_eff ≈ 0.02, β_eff values for 64/128 ch filling in). This means **C\*(NTf2) < 32 channels while C\*(FSI) ≈ 52.6** — the critical capacity depends on the anion, i.e. on κ. **The substitution law is system-specific and predictive**: given a system's complexity, C\* = λκ/s\^m is determined (Fig. unified_law_mechanism, center), and the transition point shifts with composition — shown in Fig. unified_law_cross (left: same-capacity 32-ch gap; right: β_eff with C\* shifting). At 64 ch Pyr14-NTf2 shows a *negative* gap at N=15 (−23.8 meV/atom, i.e. scalar beats equivariant) with a positive N=45 value — indicating the equivariance advantage is weak/inconsistent for this IL (consistent with the anion-insensitive, cation-driven force findings; B5). The 32-ch evidence is decisive for κ-dependence; the 64/128-ch NTf2 values (incl. the sign flip) warrant multi-seed confirmation and are reported honestly. The mechanism figure also depicts the symmetry discount κ_eff = κ/s\^m and the unified small→large-scale spectrum.
