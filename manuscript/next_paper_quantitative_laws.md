...
*Verification status (honest)*: the three limits reduce correctly; the cross-system trend (EMIM ≈ 0) is confirmed; C\* is located in the (32, 128) interval from the existing 32/128-ch data. The sharp-vs-smooth distinction requires intermediate capacities (64/96 ch), but energy-only training under e3nn 0.6 underfits (64-ch l0 10471/7301 meV vs e3nn 0.5's 271/246), so this final refinement needs a legacy e3nn 0.5 environment (future work). The candidate law is thus strong (limits, mechanism, cross-system trend, C\* interval) with one open refinement.
