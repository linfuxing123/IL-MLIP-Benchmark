...
## Core contribution (a law, not a study)

We promote "how much is equivariance worth" from a qualitative question to a **quantitative, predictive law**:
1. **A single unified scaling law** — L(N,D,C;s,κ) = L∞ + A_s·N^(−α(s)) + B_s·D^(−β_eff(C)) — that unifies 14 empirical substitutability laws into one expression, and merges the small-scale capacity phase transition (ours) with the large-scale scaling evidence (Ngo & Ravanbakhsh, ICLR 2026).
2. **A mechanistic derivation** (§3.2g): β_eff is derived — not fitted — from the competition between representation capacity and effective complexity (κ_eff = κ/s^m), giving β_eff = β_max·σ((C − κ/s^m)/w) as a two-state crossover (critical capacity C\* ≈ 52.6 ch).
3. **Cross-system predictive power verified**: a second IL (Pyr14-NTf2) already substitutes at 32 ch (β_eff ≈ 1.07), so C\*(NTf2) < 32 < C\*(FSI) ≈ 52.6 — the critical capacity is set by chemical composition κ, so the law predicts C\* = λκ/s^m for a given system **without training**.

## Abstract

Equivariant neural network interatomic potentials dominate ionic-liquid (IL) modeling, yet **how much their advantage is worth** — and whether that value can be substituted by cheaper resources (data, capacity) — has remained qualitative. Using a controlled MACE maximum-angular-momentum ablation (l_max = 0 vs 2) across 8 ILs, multiple data sizes (N = 15–59), channel capacities (32/64/128), and a bulk 2-ion-pair dataset, we derive quantitative laws of equivariance substitutability. (i) **Data-substitution power law**: the equivariance energy gap decays as gap ~ N^−1.49 with data when capacity is sufficient (128 ch), but saturates at a constant ~65 meV/atom when capacity is scarce (32 ch) — data substitutes equivariance only when capacity is available. (ii) **Data-efficiency law**: under capacity scarcity, equivariant models provide 4.0–5.8× data efficiency (collapsing to 1.2× at ample capacity). (iii) **Capacity-substitution law**: 4× capacity replaces 61–91% of the equivariance need. (iv) **Radial-dominance law (forces)**: force error is 59–99% radial across 8 ILs (weakening to ~59% in bulk); equivariance primarily improves force magnitude, not direction. (v) **Cation-modulated force gaps**: mean force gap ranks EMIM (+1709 meV/Å) > BMIM (+333) > Pyr14 (−90); anion grouping shows no ordering — force gaps are cation-driven. (vi) **PAC calibration**: the empirical substitution rate is 3–4× the worst-case bound. (vii) **Energy generalization theory**: energy accuracy = f(force supervision, training coverage, test span); forces generalize robustly, energy is sampling-sensitive. The substitutability laws unify into a single scaling law — **L(N, D, C; s, κ) = L∞ + A_s·N^(−α(s)) + B_s·D^(−β_eff(C))**, with α, β_max increasing in symmetry degree s (merging the large-scale scaling evidence of Ngo & Ravanbakhsh, ICLR 2026) and β_eff governed by a **capacity-dependent substitution phase transition with critical capacity C\* ≈ 52.6 channels** — empirically verified at 32/64/128 ch (β_eff: 0.02 → 1.19 → 1.31), and cross-system verified on a second IL (Pyr14-NTf2). These laws make equivariance substitutability quantitative, predictive, and unified, providing decision rules for MLIP deployment on ILs.
...
### 3.2g Mechanistic derivation of β_eff (from representation capacity vs effective complexity)

To promote β_eff from an empirical fit to a derived quantity, we model the competition between representation capacity and effective complexity.

**Setup.** Let the equivariant layer have channel capacity C (its hidden_irreps dimension), and let s be the symmetry degree (l_max). Symmetry constrains the model to a symmetric subspace: the *effective* intrinsic complexity the network must resolve is reduced to κ_eff = κ / s^m (m ≥ 1; echoes "symmetry controls the effective number of parameters", arXiv:2502.05300). The network can only *express* the symmetric subspace (and thus benefit from the symmetry prior) if its representation capacity C meets or exceeds κ_eff.

**Two-phase competition.** Equivariance provides a symmetry prior that reduces the number of independent functions the data must teach. If the network cannot represent the symmetric subspace (C < κ_eff), the prior is unavailable — the network falls back to a scalar-like solution and added data learns no symmetric structure (gap constant, β_eff = 0). If the network *can* represent the subspace (C > κ_eff), the prior is active — data only needs to fit the residual, so the gap decays with data (β_eff > 0).

**Sigmoid as a two-state crossover.** The transition between "prior inactive" (β_eff = 0) and "prior active" (β_eff = β_max) is a two-state crossover of representation capacity crossing the effective-complexity threshold. For a smooth transition across the representation spectrum (capacity spans insufficient→sufficient continuously), the occupation of the "active" state follows a logistic function of C − κ_eff:
**β_eff(C) = β_max(s) · σ((C − κ_eff) / w),  κ_eff = κ/s^m,  w = C\* · σ_w**
where C\* = κ_eff is the critical capacity at which the two states are equiprobable, and σ_w (<1) is the sharpness ratio (width of the representation-occupancy crossover). This is a standard two-state (Ising-like/occupation) crossover — not an arbitrary ansatz — and it reproduces the three limits.

**Why the exponents are higher.** When the prior is active (β_eff = β_max), data only fits the residual, so the gap improves faster with data (β_max > β for the scalar baseline), consistent with the large-scale exponents α, β increasing in s (Ngo).

**Testable corollaries.** (i) C\* = λκ/s^m grows with intrinsic complexity κ and shrinks with symmetry s; (ii) simple ILs (small κ) have small C\* (EMIM-BF4 gap ≈ 0 ✓); (iii) higher-order equivariance (large s) lowers C\*.
...
(Table β_eff are measured two-point values; the sigmoid fit gives β_max = 1.313 — consistent.)
...
**Cross-system test (κ dependence).** To test whether the critical capacity is set by chemical complexity κ (as the law predicts C\* = λκ/s\^m), we repeat the 32/64/128-ch capacity sweep on a second IL, Pyr14-NTf2 (same Pyr14 cation, different NTf2 anion vs the FSI of the reference). **At 32 ch, Pyr14-NTf2 already shows data substitution: gap 52.7 → 16.2 meV/atom (β_eff ≈ 1.07)**, in sharp contrast to Pyr14-FSI at 32 ch (gap 66.5 → 65.0, β_eff ≈ 0.02). This means **C\*(NTf2) < 32 channels while C\*(FSI) ≈ 52.6** — the critical capacity depends on the anion, i.e. on κ. **The substitution law is system-specific and predictive**: given a system's complexity, C\* = λκ/s\^m is determined (Fig. unified_law_mechanism, center), and the transition point shifts with composition — shown in Fig. unified_law_cross (left: same-capacity 32-ch gap; right: β_eff with C\* shifting). At 64/128 ch Pyr14-NTf2 shows *negative* gaps (64ch −23.8 → +20.3; 128ch +2.1 → −4.4 meV/atom) — i.e. equivariance is often *worse* than scalar for this IL. **The full sweep therefore shows NTf2's equivariance energy advantage is weak and inconsistent** (unlike FSI's clean positive 0.02 → 1.19 → 1.31 transition), consistent with the cation-driven force findings (B5) where the Pyr14 force gap is negative. The 32-ch evidence is decisive for κ-dependence; the 64/128-ch sign flips are reported honestly and warrant multi-seed confirmation.

**Cross-system (multi-IL): C\* depends on the system, not a single variable.** Sweeping a second cation-bearing IL, EMIM-NTf2, gives 32 ch gap 127.5 → 130.3 (β_eff ≈ −0.02, constant — no substitution) and 64 ch gap 33.7 → 26.6 (β_eff ≈ 0.21, weak decay). The **full multi-IL comparison** shows the 32-ch β_eff varies by system (Pyr14-FSI 0.02, EMIM-NTf2 −0.02, Pyr14-NTf2 1.07) — i.e. **32 ch sits in the crossover band whose location (and hence C\*) is set by the specific system**, not by a single anion/cation label. Crucially, EMIM-NTf2 needs C\* > 64 (its 64-ch β is only 0.21, far below FSI's 1.19 at 64 ch), so **C\* grows with the system's effective complexity κ** — consistent with C\* = λκ/s^m and supported across systems (FSI C\*≈52.6, NTf2 <32, EMIM-NTf2 >64). Cross-architecture validation (NequIP) is deferred to future work: it requires a force-annotated dataset (the present MACE data are energy-only and the e3nn version drift blocks force injection) — noted honestly, not filled.
...
**The law's meaning and domain.** Beyond ionic liquids, the result states a general principle: *symmetry is a substitutable resource — its value is set by a competition between representation capacity and effective complexity, with a critical capacity C\* = λκ/s^m above which cheaper resources (data) substitute it*. Because this depends only on (κ, s, C) and not on the specific chemistry, the law should transfer to *any* symmetric inductive bias (rotations, permutations, gauge, group-equivariant networks) and *any* task where a symmetry prior competes with data — e.g. equirectangular/geometric ML, graph/permutation-invariant models, and symmetry-constrained physics. The mechanistic derivation (§3.2g) frames β_eff as a two-state crossover, making C\* a *derived* (predictable) rather than fitted quantity — so the framework is a candidate  **general "symmetry-substitution law"** that the field can apply, not a single-dataset observation.
...
- unified_law_cross.png (cross-system: same-capacity 32ch FSI vs NTf2 gap; β_eff vs capacity with C* shifting by composition)
